...
char ssid[] = "Studio 5";
char password[] = "jubensha";

// LED strip configuration
#define LED_PIN 6
#define LED_COUNT 819
...
int underR = 0, underG = 0, underB = 0; 
...
void loop() {
  ...
  if (client) {
    ...
    // Match the request
    if (request.indexOf("/effect10") != -1) { //forest
      resetEffects();
      isForest = true;
    } else if (request.indexOf("/effect11") != -1) { //jungle
      resetEffects();
      isJungle = true;
    } else if (request.indexOf("/effect12") != -1) { //ocean
      resetEffects();
      isOcean = true;
    } else if (request.indexOf("/effect13") != -1) { //boss
      resetEffects();
      isBoss = true;
    } else if (request.indexOf("/effect14") != -1) { //lab
      resetEffects();
      isLab = true;
    } else if (request.indexOf("/effect15") != -1) { //toxic
      resetEffects();
      isToxic = true;
    } else if (request.indexOf("/effect16") != -1) { //rainbow
      resetEffects();
      isRainbow = true;
    } else if (request.indexOf("/effect1") != -1) { //sunrise
      resetEffects();
      effect1(); 
    } else if (request.indexOf("/effect2") != -1) { //daytime
      resetEffects();
      effect2();
    } else if (request.indexOf("/effect3") != -1) { //dusk
      resetEffects();
      effect3();
    } else if (request.indexOf("/effect4") != -1) { //night
      resetEffects();
      effect4();
    } else if (request.indexOf("/effect5") != -1) { //cloudy
      resetEffects();
      isCloudy = true;
    } else if (request.indexOf("/effect6") != -1) { //stormy
      resetEffects();
      isStormy = true;
    } else if (request.indexOf("/effect7") != -1) { //rainy
      resetEffects();
      isRainy = true;
    } else if (request.indexOf("/effect8") != -1) { //fire
      resetEffects();
      isFire = true;
    } else if (request.indexOf("/effect9") != -1) { //tavern
      resetEffects();
      isTavern = true;
      /*} else if (request.indexOf("/setcolor") != -1) { //solid color
      resetEffects();
      handleSetColor(request);*/
    } else if (request.indexOf("/setcolorO") != -1) { //solid color
      resetEffects();
      handleSetColorO(request);
    } else if (request.indexOf("/setcolorU") != -1) { //solid color
      resetEffects();
      handleSetColorU(request);
    } else if (request.indexOf("/setcolorB") != -1) { //solid color
      resetEffects();
      handleSetColorU(request);  
      handleSetColorO(request); 
    } else if (request.indexOf("/setfog") != -1) { //fog
      resetEffects();
      handleSetFog(request);
    } else if (request.indexOf("/setFan") != -1) { //fan
      resetEffects();
      //handleSetFog(request);
    }
...
void effect1() {
  ...
  fadeToColorO(currentG, currentR, currentB, startG, startR, startB, 2000);
  fadeToColorO(startG, startR, startB, endG, endR, endB, 2000);
  currentG = endG;
  currentR = endR;
  currentB = endB;
}
...
void effect2() {
  ...
  fadeToColorO(currentG, currentR, currentB, endG, endR, endB, 2000);
  currentG = endG;
  currentR = endR;
  currentB = endB;
}
...
void cloudyEffect() {
  ...
  if (isDim) {
    ...
    fadeToColorO(currentG / 10, currentR / 10, currentB / 10, currentG, currentR, currentB, 2000);
    isDim = false;
    num = random(0, 30000);
...
void stormyEffect() {
  ...
  fadeToColorO(currentG, currentR, currentB, 32, 32, 96, 100);
...
void handleSetFog(String request) {
  int colorStart = request.indexOf("color=") + 6;
  int colorEnd = request.indexOf(" ", colorStart);
  if (colorEnd == -1) {
    colorEnd = request.length();
  ...
  setFog(colorStr); // 1000 ms fade duration
}

void setFog(String value)
{
  if (value == "1")
  {
...
void handleSetColorO(String request) {
  int colorStart = request.indexOf("color=") + 6;
  int colorEnd = request.indexOf(" ", colorStart);
  if (colorEnd == -1) {
    colorEnd = request.length();
  ...
  fadeToColorO(currentG, currentR, currentB, newG, newR, newB, 1000); // 1000 ms fade duration
...
void handleSetColorU(String request) {
  int colorStart = request.indexOf("color=") + 6;
  int colorEnd = request.indexOf(" ", colorStart);
  if (colorEnd == -1) {
    colorEnd = request.length();
  ...
  fadeToColorU(underG, underR, underB, newG, newR, newB, 1000); // 1000 ms fade duration
...
void fadeToColorO(int g1, int r1, int b1, int g2, int r2, int b2, int duration) {
  int steps = 50; // Number of steps in the fade
  int delayTime = duration / steps; // Delay between each step
...
void fadeToColorU(int g1, int r1, int b1, int g2, int r2, int b2, int duration) {
  int steps = 50; // Number of steps in the fade
  int delayTime = duration / steps; // Delay between each step
...
void setColorO(int g, int r, int b) {
  for (int i = 0; i < strip1.numPixels(); i++) {
    strip1.setPixelColor(i, strip1.Color(r, g, b)); //switched rgb back
  }
  strip1.show();
}

void setColorU(int g, int r, int b) {
  for (int i = 0; i < strip2.numPixels(); i++) {
    strip2.setPixelColor(i, strip2.Color(r, g, b)); //switched rgb back
  }
  strip2.show();
}
